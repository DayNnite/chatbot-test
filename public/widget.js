      ...
      window.AIChatWidget = {
        init: function(config) {
          // Use local API in dev, prod API in prod
          var isLocal = (
            window.location.hostname === 'localhost' ||
            window.location.hostname === '127.0.0.1' ||
            window.location.protocol === 'file:'
          );
          var apiHost = isLocal ? 'http://localhost:3000' : 'https://testmyprompt.com';
          ...
          if (!clientSuppliedSettings) {
            // Start fetching settings from the server - always use testmyprompt.com
        ...
        initWithSettings: function(config) {
          ...
          const defaultSettings = {"theme":"light","themeColor":"#d97706","showFirstMessage":true,"firstMessage":"Have any questions?  Ask them to me :)","logoUrl":"https://premieralliancegrp.com/wp-content/uploads/2022/07/Premier-Alliance-Logo-Standard.png","logoWidth":100,"logoCropPosition":{"x":50,"y":50},"logoScale":1,"widgetLogoCropPosition":{"x":50,"y":50},"widgetLogoScale":1,"useLogoForAvatar":false,"useLogoForMinimizedWidget":false,"dailyLimit":200,"widgetTitle":"Premier Alliance","width":400,"height":500,"minimizedSize":"large","placement":"bottom-right","inputPlaceholder":"Type your message...","typingIndicator":"typing...","chatBubble":{"enabled":true,"message":"Hi there! Need any help?","delaySeconds":2}};
          
          // Merge default DB settings with any overrides supplied at init()
          const finalSettings = { ...defaultSettings, ...config };
          
          // Ensure baseUrl is set (default to testmyprompt.com if not provided)
          ...
          iframeContainer.style.cssText = `
            ...
            pointer-events: auto;
            background-color: ${finalSettings.theme === 'dark' ? '#111419' : 'white'};
          `;
          ...
          const iframeSrc = `${iframeBaseUrl}/widget/${widgetId}?theme=${finalSettings.theme}&themeColor=${encodeURIComponent(finalSettings.themeColor)}&logoUrl=${encodeURIComponent(finalSettings.logoUrl || '')}&logoWidth=${finalSettings.logoWidth}&logoCropX=${finalSettings.logoCropPosition?.x || 50}&logoCropY=${finalSettings.logoCropPosition?.y || 50}&logoScale=${finalSettings.logoScale || 1}&widgetLogoCropX=${finalSettings.widgetLogoCropPosition?.x || 50}&widgetLogoCropY=${finalSettings.widgetLogoCropPosition?.y || 50}&widgetLogoScale=${finalSettings.widgetLogoScale || 1}&useLogoForAvatar=${finalSettings.useLogoForAvatar}&inputPlaceholder=${encodeURIComponent(finalSettings.inputPlaceholder)}&typingIndicator=${encodeURIComponent(finalSettings.typingIndicator)}&widgetTitle=${encodeURIComponent(finalSettings.widgetTitle)}&firstMessage=${encodeURIComponent(finalSettings.firstMessage)}&showFirstMessage=${finalSettings.showFirstMessage}`;          // console.log('AIChatWidget: Loading iframe source:', iframeSrc);
